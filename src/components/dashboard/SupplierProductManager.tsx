...
export function SupplierProductManager() {
  ...
  const [form, setForm] = useState({
    name: "", description: "", price: "", unit: "kg", stock_quantity: "", category_id: "", image_url: "",
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async (product: any) => {
      if (editingProduct) {
        ...
        const { error } = await supabase.from("products").insert({ ...product, supplier_id: profile!.id });
        if (error) throw error;
      }
    },
    onSuccess: () => {
  ...
  const resetForm = () => {
    setForm({ name: "", description: "", price: "", unit: "kg", stock_quantity: "", category_id: "", image_url: "" });
  ...
  const openEdit = (product: any) => {
    ...
    setForm({
      ...
      unit: product.unit, stock_quantity: String(product.stock_quantity),
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    saveMutation.mutate({
      ...
      unit: form.unit, stock_quantity: parseInt(form.stock_quantity),
              ...
              <div key={p.id} className="flex items-center justify-between py-3 border-b last:border-0">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-sm text-muted-foreground">${Number(p.price).toFixed(2)}/{p.unit}</span>
                    <span className="text-sm text-muted-foreground">Stock: {p.stock_quantity}</span>
                  </div>
                </div>
